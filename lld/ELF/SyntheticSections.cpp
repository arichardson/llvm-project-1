  ...
  for (FileGot &got : gots) {
    ...
    for (const std::pair<GotEntry, size_t> &p : got.local16) {
      ...
      mainPart->relaDyn->addReloc({target->relativeRel, this, offset,
                                   DynamicReloc::RelativeWithTargetVA,
...
int64_t DynamicReloc::computeAddend() const {
  switch (kind) {
  case RelativeNoSymbol:
    assert(sym == nullptr);
    return addend;
  case AgainstSymbol:
    assert(sym != nullptr);
    return addend;
  case RelativeWithTargetVA:
...
void RelocationBaseSection::addRelativeReloc(
  ...
  addReloc(DynamicReloc::RelativeWithTargetVA, dynType, inputSec, offsetInSec,
