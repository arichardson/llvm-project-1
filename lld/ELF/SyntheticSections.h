...
public:
  enum Kind {
    /// This dynamic relocation is a relative relocation that does not
    /// reference a symbol (#sym must be nullptr) and uses #addend as the value
    /// of computeAddend().
    RelativeNoSymbol,
    /// This dynamic relocation will not be against the symbol but will instead
    /// be a relative relocation that simply adds the load address. This means
    /// we will write InputSection::getRelocTargetVA() for #sym + #addend as the
    /// final Elf_Rel/Elf_Rela addend.
    RelativeWithTargetVA,
    /// This dynamic relocation references symbol #sym from the dynamic symbol
    /// table and uses #addend as the value of computeAddend().
    AgainstSymbol,
    /// This dynamic relocation references symbol #sym from the dynamic symbol
    /// table and uses InputSection::getRelocTargetVA() + #addend for the
    /// final Elf_Rel/Elf_Rela addend.
    /// This can be used for relocations that write the symbol VA as the addend
    /// (e.g. R_MIPS_TLS_TPREL64) but still reference the symbol.
    AgainstSymbolWithTargetVA,
    /// This dynamic relocation is used by the MIPS multi-GOT implementation. It
    /// relocates addresses of 64kb pages that lie inside the output section.
    MipsMultiGotPage,
  };
  ...
  DynamicReloc(RelType type, const InputSectionBase *inputSec,
      ...
      : type(type), sym(nullptr), inputSec(inputSec), offsetInSec(offsetInSec),
        kind(RelativeNoSymbol), expr(R_ADDEND), addend(addend) {}
...
public:
  ...
  /// Add a relative relocation using the target address of \p sym as the addend
  /// if \p sym is non-preemptible. Otherwise add a relocation against \p sym.
  void addRelativeRelocIfNonPreemptible(RelType dynType, InputSectionBase *isec,
