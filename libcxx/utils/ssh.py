...
def main():
    ...
    # HACK: (when --test-executables is not passed)
    ...
    # test.
    # TODO: remove this once all users pass --test-executables
    def isTestExe(args, exe):
        if args.test_executables:
            return exe in args.test_executables and os.path.exists(exe)
        return exe.endswith('.tmp.exe') and os.path.exists(exe)
    isTestExe = lambda exe: isTestExe(args, exe) and os.path.exists(exe)
