...
; RUN: opt -S -passes=instsimplify < %s| FileCheck %s --allow-unused-prefixes --check-prefixes=ALL,INSTSIMPLIFY,INTEGRAL,INTEGRAL-IS
; RUN: opt -S -passes=instcombine < %s | FileCheck %s --allow-unused-prefixes --check-prefixes=ALL,INSTCOMBINE,INTEGRAL,INTEGRAL-IC
...
; RUN:  FileCheck %s --allow-unused-prefixes --check-prefixes=ALL,INSTSIMPLIFY,NONINTEGRAL,NONINTEGRAL-IS
; RUN: sed -e 's/p:64:64:64:32/p:64:64:64:32-ni:1/g' %s | opt -S -passes=instcombine | \
; RUN:  FileCheck %s --allow-unused-prefixes --check-prefixes=ALL,INSTCOMBINE,NONINTEGRAL,NONINTEGRAL-IC
...
; NONINTEGRAL-NEXT:    ret i128 ptrtoint (i8 addrspace(1)* getelementptr (i8, i8 addrspace(1)* inttoptr (i16 -1 to i8 addrspace(1)*), i32 -65536) to i128)
;
...
; INTEGRAL-LABEL: define {{[^@]+}}@constant_fold_ptrtoint_of_gep_of_inttoptr_trunc() {
; INTEGRAL-NEXT:    ret i16 1
;
; NONINTEGRAL-LABEL: define {{[^@]+}}@constant_fold_ptrtoint_of_gep_of_inttoptr_trunc() {
; NONINTEGRAL-NEXT:    ret i16 ptrtoint (i8 addrspace(1)* getelementptr (i8, i8 addrspace(1)* inttoptr (i128 65539 to i8 addrspace(1)*), i32 -2) to i16)
;
